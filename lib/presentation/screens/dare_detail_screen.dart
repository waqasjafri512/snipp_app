...
class _DareDetailScreenState extends State<DareDetailScreen> {
  ...
  Future<void> _uploadProof(BuildContext context, AppTheme theme) async {
  ...
  Widget _buildAvatarSmall(int idx, AppTheme theme, bool isDark, {String? avatarUrl, String? username}) {
    return Container(
      width: 34,
      height: 34,
      decoration: BoxDecoration(
        ...
        shape: BoxShape.circle,
        color: (avatarUrl != null) ? (isDark ? Colors.white10 : Colors.grey[200]) : null,
  ...
  Widget _buildOutlineButton(String label, AppTheme theme, bool isDark) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      decoration: BoxDecoration(
        border: Border.all(color: theme.primaryStart.withOpacity(0.28), width: 2),
        borderRadius: BorderRadius.circular(14),
      ),
      child: Text(
        label,
  ...
  Widget _buildFullWidthOutlineButton(String label, AppTheme theme, bool isDark, {VoidCallback? onTap}) {
  ...
  Widget _buildStatItem(IconData icon, String value, String label, AppTheme theme, bool isDark, {bool isLast = false, Color? iconColor}) {
    return Expanded(
      child: Container(
        padding: const EdgeInsets.symmetric(vertical: 12, horizontal: 4),
        decoration: BoxDecoration(
          border: isLast ? null : Border(right: BorderSide(color: isDark ? Colors.white10 : const Color(0xFFF0EEFF), width: 1)),
        ),
        child: Column(
          children: [
            Icon(icon, color: iconColor ?? theme.primaryStart, size: 20),
            const SizedBox(height: 4),
            Text(
              value,
              style: GoogleFonts.plusJakartaSans(
  ...
  Widget _buildCommentItem(String user, String text, String time, int likes, int idx, AppTheme theme, bool isDark, {Map<String, dynamic>? reply, String? avatarUrl}) {
    return Padding(
      padding: const EdgeInsets.only(bottom: 16),
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _buildAvatarSmall(idx, theme, isDark, avatarUrl: avatarUrl, username: user),
          const SizedBox(width: 10),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                ...
                Padding(
                  ...
                  child: Row(
                    children: [
                      Text(time, style: GoogleFonts.plusJakartaSans(fontSize: 11, color: isDark ? Colors.white54 : AppColors.muted)),
                      const SizedBox(width: 14),
                      Icon(Icons.favorite_rounded, size: 12, color: const Color(0xFFEF4444).withOpacity(0.7)),
                      const SizedBox(width: 4),
                      Text('$likes', style: GoogleFonts.plusJakartaSans(fontSize: 11, color: isDark ? Colors.white54 : AppColors.muted, fontWeight: FontWeight.w600)),
                      const SizedBox(width: 14),
                      Text('Reply', style: GoogleFonts.plusJakartaSans(fontSize: 11, color: theme.primaryStart, fontWeight: FontWeight.w700)),
                    ],
                  ),
                ),
                if (reply != null) ...[
                  ...
                  Row(
                    children: [
                      const SizedBox(width: 8),
                      _buildAvatarSmall(reply['idx'], theme, isDark),
                      const SizedBox(width: 8),
                      Expanded(
                        child: Container(
